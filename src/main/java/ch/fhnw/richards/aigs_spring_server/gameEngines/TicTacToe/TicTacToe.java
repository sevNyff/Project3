...
public class TicTacToe implements GameEngine {
	 ...
	 * @return null if still playing, 0 if cat's game, otherwise 1 or -1 for the winner
	 */
	public static Long getWinner(long[][] board) {
			int numEmptyCells = 0;
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					if (board[i][j] == 0) numEmptyCells++;
				}
			}
			...
			for (int i = 0; i < 3; i++) {
				...
				colPiece = board[i][0];
				for (int j = 1; j < 3; j++) {
					if (board[j][i] != rowPiece) rowPiece = 0;
					if (board[i][j] != colPiece) colPiece = 0;
				}
				if (rowPiece != 0) winner = rowPiece;
				if (colPiece != 0) winner = colPiece;
			}
			
			long diag1 = board[0][0];
			long diag2 = board[2][0];
			for (int i = 1; i < 3; i++) {
				if (board[i][i] != diag1) diag1 = 0;
				if (board[2-i][i] != diag2) diag2 = 0;
			}
			if (diag1 != 0) winner = diag1;
			if (diag2 != 0) winner = diag2;
			
			if (numEmptyCells == 0) return 0l;
			else if (winner != 0) return winner;
			else return null;
	}
}
